...
Here we will perform RNA-Seq analysis using the data from the *airway* package. The airway data comes from the paper by [@himes2014rna]. It is 8 RNA-seq samples from human airway smooth muscle cells: a sample treated with dexamethasone and an untreated control sample for 4 cell lines.
...
counts_scal_PCA
```
For plotting, we can select just the dimensions for the samples with `pivot_sample`.
...
In the heatmap we can see the samples cluster into the two groups, treated and untreated, for three of the cell lines but one cell line (N080611) clusters away from the others.
...
If we just want a table of DE genes we can use `pivot_transcript`.
...
We can write out our DE genes to a file that can be loaded into e.g. Excel. `write_tsv` will create a tab-separated file.
...
We can count how many DE genes there are using the significant column.
...
- Other useful plots for assessing RNA-Seq data are density plots, volcano plots, stripcharts and heatmaps. These can all be made in a 'tidy' way.
...
4. Check expression of pasilla gene (Ensembl gene id: FBgn0261552)
