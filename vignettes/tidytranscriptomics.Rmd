...
There are many steps involved in analysing an RNA sequencing experiment. Analysing an RNA sequencing experiment begins with sequencing reads. These are aligned to a reference genome, then the number of reads mapped to each gene can be counted. This results in a table of counts, which is what we perform statistical analyses on in R. While mapping and counting are important and necessary tasks, today we will be starting from the count data and getting stuck into analysis.
...
Here we will perform RNA sequencing analysis using the data from the *airway* package. The airway data comes from the paper by [@himes2014rna]. It is 8 RNA-seq samples from human airway smooth muscle cells: a sample treated with dexamethasone and an untreated control sample for 4 cell lines.

### Setting up the data

The airway RNA sequencing data is stored as a *RangedSummarizedExperiment* object. We can convert a SummarizedExperiment object into a tidybulk tibble, which is the tidyverse table format.
...
2. How many differentially expressed genes are there for treated vs untreated (FDR < 0.05)? What is the top differentially expressed gene by P value? 
