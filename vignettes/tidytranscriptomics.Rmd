...
Here we will perform RNA-Seq analysis using the data from the *airway* package. It has 8 samples from human airway smooth muscle cells: a sample treated with dexamethasone and an untreated control sample for 4 cell lines.

### Setting up the data

The airway RNA-Seq data is stored as a *RangedSummarizedExperiment* object. We can convert a SummarizedExperiment object into a tidybulk tibble, which is the tidyverse table format.
...
- Other useful plots for assessing RNA-Seq data are density plots, box plots, volcano plots, stripcharts and heatmaps. These can all be made in a 'tidy' way.
...
Try to apply what you've learned to another RNA-seq dataset. The data is was generated from the pasilla package which obtained the data from the paper by Brooks et al. 2010. Here we provide it as a SummarizedExperiment object. The dataset has 7 samples from Drosophila (fruitfly): 3 treated with siRNA knockdown of the pasilla gene and 4 untreated controls.
Load the data and create the tidybulk object with:
```{r}
data("pasilla", package = "tidytranscriptomics")
counts_tt <- 
	pasilla %>% 
  tidybulk(sample, symbol, `count normalised adjusted`)
```
...
2. How many differentially expressed genes are there?
3. What code can generate a heatmap of variable genes for pasilla (starting from count_scaled)?
4. What code can generate an interactive volcano plot that has gene symbols showing on hover?
5. What code can generate a heatmap of the top 100 differentially expressed genes?
