...
```{r}
counts_annot <- ensembl_to_symbol(counts_airway, feature)
counts_annot
```
...
```{r}
counts_tt <- counts_annot %>% 
  mutate(sample=str_remove(sample, "SRR1039"))
```

With tidyverse, all above operations can be linked with the `%>%`, as shown below, so no temporary variables need to be created, less typing is required, and the steps can be seen more clearly. 
...
2. Another way to check the distributions of the counts in the samples is with box plots. Adapt the code above to create box plots of the counts before and after scaling, similar to what was done for the density plots. How does the before and after compare?
...
1. Colour the MDS plot i) by treatment, ii) by cell line. Try using `shape` inside `aes` with the dex and/or cell variables.
