...
author:
  - Maria Doyle^[Peter Mac, maria.doyle@petermac.org]
  - Stefano Mangiola^[WEHI, mangiola.s@wehi.edu.au]
...
The airway RNA-Seq data is stored in the airway package as a *RangedSummarizedExperiment* object. We'll extract the counts into a table and do the same for the sample information. Tables are often the format of the data at the start of an RNA-seq analysis. We'll use the tidyverse table format, which is called a tibble, which we can create using the tidyverse *tibble* package.

In this workshop we will be using the tidyverse pipe `%>%`. This 'pipes' the output from the command on the left into the command on the right/below. Using the pipe is not essential but it reduces the amount of code we need to write when we have multiple steps (as we'll see later). It also can make the steps clearer and easier to see.  For more details on the pipe see [here](https://r4ds.had.co.nz/pipes.html). We can use the pipe `%>%` when we load the *dplyr* tidyverse library.
...
First we will convert the counts into long format (tidy format) using `pivot_longer` from the *tidyr* tidyverse package. *This format is key for working according to the tidy data paradigm and using ggplot2.*
...
We can shorten the sample names. We can remove the SRR1039 prefix that's present in all of them, as shorter names can fit better in some of the plots we will create. We can use `mutate` from the tidyverse *dplyr* package together with `str_replace` from the tidyverse *stringr* package to remove the SRR1039 string from the sample column.
...
Note that all the above operations could be linked with the `%>%`, so no temporary variables have to be created.
...
However, to explicitly and/or temporarly drop them from the data set is useful for data visualisation. You can specify criteria to filter and tidybulk can also use the *edgeR* `filterByExpr` function described [here](https://f1000research.com/articles/5-1408). By default, this will keep genes with ~10 counts in a minimum number of samples, the number of the samples in the smallest group. In this dataset the smallest group size is 4 samples. tidybulk performs this filtering in the functions we will use `scale_abundance` and `test_differential_abundance` and we can explicitly apply it with `keep_abundant`.
...
Scaling of counts, normalisation, is performed to eliminate uninteresting differences between samples. These are differences due to sequencing depth or composition. In the tidybulk package the function `scale_abundance` generates scaled counts, it will also perform the filtering. We can choose from different normalisation methods. Here we will use the default, TMM, [@robinson2010tmm]. TMM normalisation (and most scaling normalisation methods) scale relative to one sample. 
...
Another way to check the distributions of the counts in the samples is with box plots. Box plots can make it easier to see the distributions for individual samples than density plots, especially if there are many samples. We can add a line through the median to help us see how similar (or not) the distributions are.
...
#### Exercise
Adapt the code above to create box plots of the counts before and after scaling, similar to what was done for the density plots. How does the before and after compare?

# Quality control

## Multidimensional scaling plots

By far, one of the most important plots we make when we analyse RNA-Seq data are MDS plots. An MDS plot is a visualisation of a principal components analysis, which determines the greatest sources of variation in the data. A principal components analysis is an example of an unsupervised analysis, where we don't need to specify the groups. If your experiment is well controlled and has worked well, and if there are genuine biological differences, what we hope to see is that the greatest sources of variation in the data are the treatments/groups we are interested in. It is also an incredibly useful tool for quality control and checking for outliers. We can use the tidybulk `reduce_dimensions` function to calculate the dimensions and then create an MDS plot.
...
# MDS plot
MDSdims %>%
ggplot(aes(x=Dim1, y=Dim2, colour=sample)) + 
  geom_point() +
	theme_bw()
```
...
Colour the MDS plot i) by treatment, ii) by cell line.
Try using `shape` inside `aes` with the dex and/or cell variables.
...
We can write out our differentially expressed genes to a file that can be loaded into e.g. Excel. `write_tsv`from the tidyverse *readr* package will create a tab-separated file.
...
We only have one contrast here so we could remove the suffix `_dextrt - dexuntrt` from the column headers to make the names shorter to use in the rest of the commands here, using `rename_at` from the tidyverse *dplyr* package together with `str_replace` from the tidyverse *stringr* package.
...
#### Exercise
Count how many upregulated DE genes there are Hint: use `filter` and the logFC column.  
...
Let's make a few plots to make sure everything looks good and that we haven't made a mistake in the analysis. Genome-wide plots that are useful for checking are MA plots and volcano plots. We can also use stripcharts to visualise groups of genes.
...
In addition to the genome-wide plots already discussed, it is recommended to have a look at the expression levels of the individual samples for the genes of interest, before following up on the DE genes with further lab work. We can use stripcharts to do this. These will help show if expression is consistent amongst replicates in the groups and how the expression compares to the other groups. We'll also add a box plot to show the distribution.
