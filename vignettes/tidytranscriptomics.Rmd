...
However, to explicitly and/or temporarily drop them from the data set is useful for data visualisation. Tidybulk, uses the *edgeR* `filterByExpr` function described [here](https://f1000research.com/articles/5-1408). By default, this will keep genes with ~10 counts in a minimum number of samples, the number of the samples in the smallest group. In this dataset the smallest group size is 2 samples. tidybulk performs this filtering in the functions we will use `scale_abundance` and `test_differential_abundance` and we can explicitly apply it with `keep_abundant`.
